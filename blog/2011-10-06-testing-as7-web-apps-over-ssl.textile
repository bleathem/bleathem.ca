...
h3. 2. Create a self-signed certificate
...
h3. 3. Export the certificate

Export the certificate from the keystore you just created, again using the keytool command:

<pre class="code">keytool -exportcert -alias tomcat -keystore server.jks -file as7.cer</pre>
